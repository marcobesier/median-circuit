...
void Median_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Median_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 1047;}

uint get_number_of_components() {return 13;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1010;}
...
ctx->componentMemory[coffset].templateName = "Median";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 4; i++) {
std::string new_cmp_name = "le"+ctx->generate_position_array(aux_dimensions, 1, i);
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[5]); // line circom 10
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[0]) + 1)) + 1)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
LessEqThan_2_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[0])) + 0)]].signalStart + 0],&circuitConstants[2]); // line circom 14
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 10
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[5]); // line circom 10
}
Fr_eq(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 0]); // line circom 18
...
Median_3_create(1,0,ctx,"main",0);
Median_3_run(0,ctx);
}
